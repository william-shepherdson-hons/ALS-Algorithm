use crate::models::knowledge_tracing_model;
use crate::models::hidden_markov_model;
use crate::{evaluation::em_result::EmResult, models::models::Models};
use crate::evaluation::formatted_record::FormattedRecord;
use csv::ReaderBuilder;
use std::{collections::HashMap, error::Error};

struct UserSkillSequence {
    observations: Vec<bool>,
}

struct ExpectedCounts {
    sum_initial_mastery: f64,
    sum_learned: f64,
    sum_opportunities_unknown: f64,
    sum_correct_while_known: f64,
    sum_known: f64,
    sum_correct_while_unknown: f64,
    sum_unknown: f64,
    n_sequences: usize,
}

impl ExpectedCounts {
    fn new() -> Self {
        Self {
            sum_initial_mastery: 0.0,
            sum_learned: 0.0,
            sum_opportunities_unknown: 0.0,
            sum_correct_while_known: 0.0,
            sum_known: 0.0,
            sum_correct_while_unknown: 0.0,
            sum_unknown: 0.0,
            n_sequences: 0,
        }
    }
}

type StatePair = (f64, f64);

async fn forward_pass(observations: &[bool], params: &EmResult, model: &Models) -> Vec<StatePair> {
    let n = observations.len();
    let mut alphas: Vec<StatePair> = Vec::with_capacity(n + 1);
    alphas.push((params.initial, 1.0 - params.initial));
    for &obs in observations {
        let (a_k, a_u) = *alphas.last().unwrap();
        let next = match model {
            Models::HiddenMarkovModel => {
                hidden_markov_model::calculate_forward_pair_from_marginal(a_k, params.transition, params.slip, params.guess, obs).await
            }
            Models::KnowledgeTracingModel => {
                knowledge_tracing_model::calculate_forward_pair_from_marginal_kt(a_k, params.transition, params.slip, params.guess, obs).await
            }
        };
        let mut kn = next.0.max(0.0);
        let mut un = next.1.max(0.0);
        let s = kn + un;
        if s > 0.0 {
            kn /= s;
            un /= s;
        } else {
            kn = 0.5;
            un = 0.5;
        }
        alphas.push((kn, un));
    }
    alphas
}

async fn backwards_pass(observations: &[bool], params: &EmResult, model: &Models) -> Vec<StatePair> {
    let n = observations.len();
    let mut betas: Vec<StatePair> = vec![(0.0, 0.0); n + 1];
    betas[n] = (1.0, 1.0);
    for t in (0..n).rev() {
        let next_beta = betas[t + 1];
        let obs = observations[t];
        let pair = match model {
            Models::HiddenMarkovModel => {
                hidden_markov_model::calculate_backward_pair(next_beta.0, next_beta.1, obs, params.transition, params.slip, params.guess).await
            }
            Models::KnowledgeTracingModel => {
                knowledge_tracing_model::calculate_backward_pair_kt(next_beta.0, next_beta.1, obs, params.transition, params.slip, params.guess).await
            }
        };
        betas[t] = pair;
    }
    betas
}

fn smooth_probabilities(forward: &[StatePair], backward: &[StatePair]) -> Vec<f64> {
    let n = forward.len();
    let mut smoothed = Vec::with_capacity(n);
    for t in 0..n {
        let (f_k, f_u) = forward[t];
        let (b_k, b_u) = backward[t];
        let num_k = f_k * b_k;
        let num_u = f_u * b_u;
        let denom = num_k + num_u;
        if denom > 0.0 {
            smoothed.push(num_k / denom);
        } else {
            let s = f_k + f_u;
            if s > 0.0 {
                smoothed.push(f_k / s);
            } else {
                smoothed.push(0.5);
            }
        }
    }
    smoothed
}

async fn calculate_transition_expectations(
    observations: &[bool],
    forward: &[StatePair],
    backward: &[StatePair],
    params: &EmResult,
    model: &Models,
) -> Vec<f64> {
    let n = observations.len();
    let mut xi_values: Vec<f64> = Vec::with_capacity(n);
    for t in 0..n {
        let (alpha_k, alpha_u) = forward[t];
        let (beta_k1, beta_u1) = backward[t + 1];
        let obs_next = observations[t];
        let xi = match model {
            Models::HiddenMarkovModel => {
                hidden_markov_model::calculate_transition_expectation_pair(
                    alpha_k,
                    alpha_u,
                    beta_k1,
                    beta_u1,
                    obs_next,
                    params.transition,
                    params.slip,
                    params.guess,
                )
                .await
            }
            Models::KnowledgeTracingModel => {
                knowledge_tracing_model::calculate_transition_expectation_kt(
                    alpha_k,
                    alpha_u,
                    beta_k1,
                    beta_u1,
                    obs_next,
                    params.transition,
                    params.slip,
                    params.guess,
                )
                .await
            }
        };
        xi_values.push(xi);
    }
    xi_values
}

async fn accumulate_sequence_counts(
    observations: &[bool],
    forward: &[StatePair],
    backward: &[StatePair],
    params: &EmResult,
    model: &Models,
    counts: &mut ExpectedCounts,
) {
    let smoothed = smooth_probabilities(forward, backward);
    if observations.len() > 0 {
        let p_obs_given_known = if observations[0] { 1.0 - params.slip } else { params.slip };
        let p_obs_given_unknown = if observations[0] { params.guess } else { 1.0 - params.guess };
        let prior = params.initial;
        let numerator = prior * p_obs_given_known;
        let denominator = numerator + (1.0 - prior) * p_obs_given_unknown;
        let p_known_given_first_obs = if denominator > 0.0 { numerator / denominator } else { prior };
        counts.sum_initial_mastery += p_known_given_first_obs;
    } else {
        counts.sum_initial_mastery += params.initial;
    }
    counts.n_sequences += 1;
    let xi_values = calculate_transition_expectations(observations, forward, backward, params, model).await;
    for t in 0..observations.len() {
        let p_known = smoothed[t];
        let p_unknown = 1.0 - p_known;
        counts.sum_learned += xi_values[t];
        counts.sum_opportunities_unknown += p_unknown;
        if observations[t] {
            counts.sum_correct_while_known += p_known;
            counts.sum_correct_while_unknown += p_unknown;
        }
        counts.sum_known += p_known;
        counts.sum_unknown += p_unknown;
    }
}

fn m_step_update(counts: &ExpectedCounts) -> EmResult {
    let initial = if counts.n_sequences > 0 {
        (counts.sum_initial_mastery / counts.n_sequences as f64).min(0.99).max(0.01)
    } else {
        0.5
    };
    let transition = if counts.sum_opportunities_unknown > 0.0 {
        (counts.sum_learned / counts.sum_opportunities_unknown).min(0.99).max(0.01)
    } else {
        0.1
    };
    let slip = if counts.sum_known > 0.0 {
        (1.0 - counts.sum_correct_while_known / counts.sum_known).min(0.99).max(0.01)
    } else {
        0.1
    };
    let guess = if counts.sum_unknown > 0.0 {
        (counts.sum_correct_while_unknown / counts.sum_unknown).min(0.99).max(0.01)
    } else {
        0.2
    };
    let sum = guess + slip;
    let (guess, slip) = if sum >= 1.0 {
        let scale = 0.98 / sum;
        (guess * scale, slip * scale)
    } else {
        (guess, slip)
    };
    EmResult {
        initial,
        transition,
        slip,
        guess,
    }
}

pub async fn expectation_maximisation(model: Models, initial: EmResult, path: &str) -> Result<EmResult, Box<dyn Error>> {
    let mut reader = ReaderBuilder::new().trim(csv::Trim::All).from_path(path)?;
    let mut records: Vec<FormattedRecord> = reader.deserialize().collect::<Result<_, _>>()?;
    records.sort_by_key(|r| (r.user_id, r.skill_id, r.times_applied));
    let mut sequences: HashMap<(u32, u32), UserSkillSequence> = HashMap::new();
    for record in records {
        let key = (record.user_id, record.skill_id);
        let sequence = sequences.entry(key).or_insert_with(|| UserSkillSequence { observations: Vec::new() });
        sequence.observations.push(record.correct == 1);
    }
    const MAX_ITERATIONS: usize = 1000;
    const TOLERANCE: f64 = 1e-4;
    let mut params = initial;
    println!("Starting EM with {} sequences", sequences.len());
    println!("Initial params: L0={:.4}, T={:.4}, S={:.4}, G={:.4}", params.initial, params.transition, params.slip, params.guess);
    for iteration in 0..MAX_ITERATIONS {
        let mut counts = ExpectedCounts::new();
        for (_key, sequence) in &sequences {
            let forward = forward_pass(&sequence.observations, &params, &model).await;
            let backward = backwards_pass(&sequence.observations, &params, &model).await;
            accumulate_sequence_counts(&sequence.observations, &forward, &backward, &params, &model, &mut counts).await;
        }
        let new_params = m_step_update(&counts);
        let diff = (new_params.initial - params.initial).abs()
            + (new_params.transition - params.transition).abs()
            + (new_params.slip - params.slip).abs()
            + (new_params.guess - params.guess).abs();
        println!("Iteration {}: L0={:.4}, T={:.4}, S={:.4}, G={:.4}, diff={:.6}",
            iteration, new_params.initial, new_params.transition, new_params.slip, new_params.guess, diff);
        params = new_params;
        if diff < TOLERANCE {
            println!("Converged after {} iterations", iteration + 1);
            break;
        }
    }
    Ok(params)
}
