...
pub async fn calculate_forward_pair_from_marginal_kt(
    p_known_t: f64,
    transition: f64,
    slip: f64,
    guess: f64,
    ...
    let alpha_k_next = (p_known_t * 1.0 + p_unknown_t * transition) * b_k;
    let alpha_u_next = (p_known_t * 0.0 + p_unknown_t * (1.0 - transition)) * b_u;
